...
export class MeshService extends Construct {
  ...
  constructor(scope: Construct, id: string, props: MeshServiceProps) {
    ...
    this.firewall =
      props.firewall ||
      new WebApplicationFirewall(this, "waf", {
        name: props.wafName,
    ...
    if (!props.firewall) {
      ...
      if (this.firewall.acl.rules)
        (
          this.firewall.acl.rules as Array<IResolvable | CfnWebACL.RuleProperty>
        ).push(...defaultRules);
    }
